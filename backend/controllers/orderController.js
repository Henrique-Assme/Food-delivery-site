...
const placeOrder = async (req, res) => {
    const frontend_url = process.env.NODE_ENV === "dev" ? "http://localhost:5174" : process.env.VITE_FRONT_URL;
    ...
    try {
        ...
        console.log(error);
        res.json({ success: false, message: "Error on place order" });
    }
};
...
const verifyOrder = async (req, res) => {
    ...
    try {
        ...
        console.log(error);
        res.json({ success: false, message: "Error at verify order" });
    }
};
...
const userOrders = async (req, res) => {
    try {
        ...
        console.log(error);
        res.json({ success: false, Error: "Error in userOrders" });
    }
};
